...
const CGFloat kMyMemberShipLayoutLineSpacing = 40.f;    // item间距
const CGFloat kMyMemberShipLayoutZoomScale = 1.05f;     // 缩放比例
const CGFloat kMyMemberShipLayoutMinZoomScale = 0.95f;  // 最小缩放比例

const CGFloat kMyMemberShipItemHeight = 154.0f ;
const CGFloat kMyMemberShipItemWidth = 280.0f ;

@implementation MyMemberShipLayout {
    NSInteger _index;
}
...
- (NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect {
    ...
    for (UICollectionViewLayoutAttributes *attributes in visibleItemArray) {
        ...
        //5. 缩放
        attributes.transform3D = CATransform3DMakeScale(kMyMemberShipLayoutMinZoomScale + scale * (kMyMemberShipLayoutZoomScale - kMyMemberShipLayoutMinZoomScale), kMyMemberShipLayoutMinZoomScale + scale * (kMyMemberShipLayoutZoomScale - kMyMemberShipLayoutMinZoomScale), 1);

        CGFloat beginTransparentValue = 0.6;
        
        // 是否需要透明
        if (self.needAlpha) {
            if (scale > beginTransparentValue) {
                attributes.alpha = 1.0;
            } else if (scale > 0.2) {
                attributes.alpha = (1 - beginTransparentValue) + scale;
            } else {
